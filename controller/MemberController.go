...
func (mc *MemberController) uploadAvator(context *gin.Context) {
	...
	if err!=nil{
		...
		memberService := service.MemberService{}
		path := memberService.UploadAvatar(member.Id, fileName)
		if path != "" {
			tool.Success(context,path)
			return
		}
...
func (mc *MemberController) smsLogin(ctx *gin.Context) {
	...
	if member != nil {
		...
		if err = tool.SetSess(ctx, "user_"+string(member.Id), sess); err != nil {
